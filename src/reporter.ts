...
export class Reporter {
  ...
  async report(findings: Finding[], options: ScanOptions = {}): Promise<ScanSummary> {
    ...
    if (this.format === 'markdown') {
      const md = this.toMarkdown(findings, options);
      if (options.output) {
        const fs = await import('fs');
        fs.writeFileSync(options.output, md);
        if (!this.quiet) console.log(`📄 Markdown report saved to ${options.output}`);
      } else {
        console.log(md);
      }
      return summary;
...
<div class="footer">
  ...
  private toMarkdown(findings: Finding[], options: ScanOptions): string {
    const critical = findings.filter(f => f.severity === 'critical');
    const warning = findings.filter(f => f.severity === 'warning');
    const info = findings.filter(f => f.severity === 'info');
    ...
    const lines: string[] = [
      `# 🦅 ExtVet Security Report`,
      ``,
      `**Date:** ${timestamp} | **Browser:** ${options.browserType || this.browser || 'unknown'} | **ExtVet** v${VERSION}`,
      ``,
      `## Summary`,
      ``,
      `| Severity | Count |`,
    ...
    const renderSection = (title: string, icon: string, items: Finding[]) => {
      if (items.length === 0) return;
      lines.push(`### ${icon} ${title}`, ``);
      for (const f of items) {
        lines.push(`- **${f.extension}**: ${f.message}`);
    ...
    renderSection('Warning', '🟡', warning);
    renderSection('Info', '🔵', info);

    lines.push(`---`, `*Generated by [ExtVet](https://github.com/taku-tez/ExtVet)*`);
